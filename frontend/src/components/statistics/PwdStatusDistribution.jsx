        ...
        const fetchResidents = async () => {
            try {
                ...
                residents.forEach(resident => {
                    if (resident.is_pwd === 'Yes') {
                        pwdGroups['PWD']++;
                    } else if (resident.is_pwd === 'No') {
                        pwdGroups['Not PWD']++;
                    }
                });

                const pwdData = Object.keys(pwdGroups).map(category => ({
                    name: category,
                    value: pwdGroups[category],
                }));
    ...
    const downloadChart = async () => {
        if (chartRef.current) {
            html2canvas(chartRef.current, { backgroundColor: "#1F2937" }).then(canvas => {
                ...
                link.download = "pwd_status_distribution.png";
                link.click();
            });
        }
    };
    ...
    return (
        ...
        >
            <div className="flex justify-between items-center mb-4">
                <h2 className='text-xl font-semibold text-gray-100'>PWD Status Distribution</h2>
                <button 
...
export default PwdStatusDistribution;