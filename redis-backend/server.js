...
app.post('/resident', async (req, res) => {
  ...
  try {
    ...
    // Generate QR code containing the residentId
    const qrCodeData = `Resident ID: ${residentId}`;
    const qrCode = await QRCode.toDataURL(qrCodeData); // Generate QR code as a Base64 string

    // Set student data in Redis (using object syntax for Redis v4 and above)
    const residentData = { fullname, age, purok, gender, birthdate, email, phone, civil_status, is_pwd, is_aVoter, employment_status, income_source, educational_level, qrCode };
    ...
    await client.hSet(`resident:${residentId}`, 'qrCode', residentData.qrCode); // Save QR code
